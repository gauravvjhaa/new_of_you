...
import { HousePlug, LogOut, Menu, ShoppingCart, UserCog } from "lucide-react";
import Logo from "../../assets/Logo_transparent_Black.png"; // ✅ Use PNG version
import {
  Link,
  useLocation,
  useNavigate,
...
function HeaderRightContent() {
  ...
  useEffect(() => {
    if (user?.id) {
      dispatch(fetchCartItems(user.id)); // ✅ Only fetch cart if user is logged in
    }
  }, [dispatch, user?.id]);
